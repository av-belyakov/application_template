...
Для красивого форматирования файлов .templ, иногда линтер IDE не верно форматирует подобные файлы, выполнить команду:
...
В _contains/contains.go_ изменить значения констант **Root_Dir** и **Application** на наименование нового приложения. Из константы **App_Environment_Name** берется наименование которое подставляется в имя переменных окружения. Например, переменная окружения **GO_APPLICATIONTEMPLATE_PASSWD** будет содержать значение _APPLICATIONTEMPLATE_ которое, в настоящий момент, присвоено константе **App_Environment_Name**.

В _.gitlab-ci.yml_ изменить значение **variables.APPLICATION_NAME** на наименование своего нового приложения.
